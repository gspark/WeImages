...
//    treeView->setIndentation(20);       // indentation
    ...
    treeView->setUniformRowHeights(
            true);           // all items has same height. Enables the view to do some optimizations
    ...
    treeView->header()->setDefaultSectionSize(
            HEADER_SIZE_DEFAULT);     // only affects sections that (QHeaderView::)Interactive or Fixed
...
void FileWidget::initLabel() {
    statLab->setSizePolicy(QSizePolicy::Preferred, QSizePolicy::Preferred);
    statLab->setMaximumWidth(STATUS_LAB_WIDTH_ITEM);
}


void FileWidget::initPathBox() {
    pathBox->setEditable(true);
//    LineEdit *edit = new LineEdit();
//    pathBox->setLineEdit(edit);
    pathBox->setToolTip(
            tr("Enter \"?\" + \"name or wildcard\"  and then press enter to automatically search. For example, enter \"?*.txt\" and press enter, it will automatically search for text files in the current directory."));
...
void FileWidget::initWidgetLayout() {
    ...
    // row 0
    gridLayout->addWidget(pathBox, 0, 0, 1, 4);

    // row 1
    gridLayout->addWidget(treeView, 1, 0, 1, 4);
...
void FileWidget::onComboBoxActivated(int index) {
    ...
    if (!QFileInfo::exists(path)) {
        if (path.compare(myComputer.name, Qt::CaseInsensitive) == 0) {
            ...
            if (path[0] == QChar('?') || path[0] == QString("\uff1f")) {
...
void FileWidget::updateStatusBar() {
    ...
    int count = currDir->entryList(
            QDir::AllEntries | QDir::NoDotAndDotDot | QDir::AllDirs | QDir::System/* | QDir::Hidden*/).count();
